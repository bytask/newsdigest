...
│  1. あなたの GitHub から intel-digest を checkout                  │
│  2. 環境変数 INTEL_API_URL / INTEL_API_KEY / XAI_API_KEY …        │
│  3. .claude/skills/intel-digest/SKILL.md を実行                   │
│       GET /api/sources ──────────────────────────┐              │
...
│  API: /api/sources  /api/digests  /api/raw  /api/health         │
...
- **秘密は 2 箇所だけ**。Worker secret（`INTEL_API_KEY`、`LINE_CHANNEL_*`）と claude.ai 環境変数。リポジトリには入れない
...
- 作成・更新・手動実行・ログ取得は Claude Code 内蔵の `RemoteTrigger` ツール（claude.ai のルーティン API）で行う → `.claude/skills/intel-digest-routine/`
...
- 機械向け `/api/*` は `Authorization: Bearer <INTEL_API_KEY>`（`/api/health` と `/api/line/webhook` を除く）
...
apps/console/
  ...
  lib/            store.ts（D1）/ auth.ts（Bearer）/ config.ts（表示名）/ format.ts
  schema.sql      D1 スキーマ（冪等）
  wrangler.jsonc  Worker 設定（name / D1 binding / vars）
.claude/skills/   intel-digest / intel-sources-review / intel-digest-routine
...
このパッケージは Piper（bytask）が 2026-07 から毎朝運用している intel-digest / intel-console を切り出したもの。運用側では AI 実行を自前 VPS の cron（headless `claude -p`）で行っているが、第三者配布にあたり **利用者自身の Claude Code ルーティン** に置き換えた。コンソールのコードはほぼ同一（ブランディングの外出し、`/api/health` 追加、LINE Webhook の secrets 名を汎用化）。
